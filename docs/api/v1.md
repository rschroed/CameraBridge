...
- response: `200 OK`
- returns the current live AVFoundation permission state observed by `camd` and the session controller in the shipped v1 slice
...
- uses the same live daemon-visible permission state exposed by `GET /v1/permissions`
